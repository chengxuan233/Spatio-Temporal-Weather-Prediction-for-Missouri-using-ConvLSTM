...
def init_normalization(tensor, output_channels=2):
    """Compute mean and standard deviation for output channels, for normalization."""
    global y_mean, y_std
    all_y = tensor[:, 0:output_channels]  # Extract output channels
    y_mean = all_y.mean(dim=(0, 2, 3))    # Mean across time, height, width
    y_std = all_y.std(dim=(0, 2, 3))      # Standard deviation across same dims

def normalize(y):
    """Normalize target tensor y using global mean and std."""
...
class DeeperConvLSTMNet(nn.Module):
    """Deep ConvLSTM-based model for spatiotemporal prediction."""
    def __init__(self, input_channels, hidden_channels, output_channels, height, width, kernel_size=3):
        super().__init__()
        padding = kernel_size // 2
        # 3D convolution encoder for spatial feature extraction
        ...
        self.convlstm = None  # Initialized lazily
    ...
    def forward(self, x):
        B, T, C, H, W = x.shape
        x = x.permute(0, 2, 1, 3, 4)  # Convert to [B, C, T, H, W] for Conv3D
        x = self.encoder(x)
        _, _, T, H, W = x.shape
        x = x.permute(0, 2, 1, 3, 4).reshape(B, T, -1)  # Flatten spatial features
...
def compute_metrics(y_true, y_pred):
    """Compute MAE and RMSE for evaluation."""
...
def load_split_pt_chunks(split_name, limit_chunks=None, verbose=True):
    """Load a list of torch tensors from the split directory."""
...
def build_dataset(tensor, output_channels=2, sequence_length=12, forecast_horizon=1):
    """Construct sequences of input/output tensors for training."""
    x_seq, y_seq = [], []
...
def run_genetic_search(train_loader, val_loader, input_channels, output_channels, H, W, device):
    """Perform genetic algorithm to find best hyperparameters."""
    if os.path.exists(HYPERPARAM_FILE):
        with open(HYPERPARAM_FILE, 'r') as f:
            return json.load(f)

    # Define individuals and fitness function for DEAP
...
if __name__ == '__main__':
    ...
    # Run or load best hyperparameters
    if os.path.exists(HYPERPARAM_FILE):
    ...
    # Save logs and plot training curve
